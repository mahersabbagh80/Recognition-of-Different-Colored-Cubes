...
### Beginner-completeness gate

Before calling a lesson ready for learner review, audit it as if the reader is an experienced software developer encountering machine learning and computer vision for the first time:

1. Name the prerequisites and link any earlier lesson that owns them.
2. Define every new ML, CV, camera, or robotics term before relying on it.
3. Explain the causal “why,” not only the repository's current value or operation.
4. Give one small concrete example before the full project-specific case.
5. Distinguish a general principle from an artifact-specific contract, configured policy, measured result, and unresolved hypothesis.
6. Trace where data enters, changes representation, is filtered, and leaves the stage.
7. State what the operation cannot prove or accomplish and name common failure modes.
8. If material is deferred, identify the exact later lesson or reference that will teach it; do not leave a silent gap.
9. Ensure the exercise has an observable result or checking key.
10. Recheck claims against current code, artifacts, and canonical project status before publication.

An overview image is an orientation aid, not a substitute for these explanations. Avoid captions that claim the image contains “every core concept” when necessary foundations or caveats live only in the prose.
...
| 0003 | [PyTorch and Model Training](lessons/0003-pytorch-and-model-training.html) | What are datasets, labels, epochs, validation, checkpoints, and fine-tuning? | `models/best.pt`, `models/README.md`, training scripts | Full beginner audit completed; learner review pending |
| 0004 | [Why the Model Fails on the Robot](lessons/0004-why-the-model-fails-on-the-robot.html) | Why can validation look good while live inference fails? | M4/M5 evaluation reports, domain-gap evidence | Full beginner audit completed; learner review pending |
| 0005 | [ONNX and TensorRT](lessons/0005-onnx-and-tensorrt.html) | Why does the model travel through `best.pt → best.onnx → best.engine`? | `models/README.md`, export and TensorRT commands | Full beginner audit completed; learner review pending |
| 0006 | [ROS 2 Perception Wiring](lessons/0006-ros-2-perception-wiring.html) | How do nodes, topics, messages, `cv_bridge`, and QoS connect the detector? | `cube_detection_node.py`, `package.xml`, launch/config files | Full beginner audit completed; learner review pending |
| 0007 | [Depth and Geometry Filtering](lessons/0007-depth-and-geometry-filtering.html) | What does the current depth predicate compute, and why is its sign not valid height reasoning? | `geometry_filter.py`, M4c1 evidence | Full beginner audit completed; known production-code defect documented; learner review pending |
| 0008 | [Evaluation and Fine-Tuning](lessons/0008-evaluation-and-fine-tuning.html) | How do we measure success and improve the model responsibly? | `evaluation/`, `docs/evaluation.md`, M3d-revived plan | Full beginner audit completed; planning inconsistencies documented; learner review pending |
| 0009 | [Project Walkthrough and Interview Practice](lessons/0009-project-walkthrough-and-interview-practice.html) | Can Maher explain the complete system, trade-offs, failures, and next steps? | Full repository and evidence | Full beginner audit completed; learner review pending |
...
| 2026-09-06 | Performed an initial beginner-focused pass across Lessons 0001–0009; this was not a full audit of every lesson. Added the reusable beginner-completeness gate and corrected the M3 ONNX smoke decoder to match the deployed center/size and pre-NMS clipping contract. |
| 2026-09-06 | Completed a full beginner audit of Lesson 0003: expanded supervision, labels, split discipline, leakage, batches, gradients, loss, optimization, augmentation, generalization, metrics, checkpoints, and transfer learning; separated historical metrics from a fresh revalidation; corrected fused-versus-raw parameter-count interpretation. |
| 2026-09-06 | Completed full beginner audits of Lessons 0004–0009. Added causal explanations and evidence boundaries for domain shift, model export, ROS 2 runtime semantics, depth geometry, evaluation design, and project synthesis. Documented the geometry depth-sign defect, the future fine-tuning plan's inconsistent sample totals, the placeholder evaluator, and the distinction between smoke gates and labelled accuracy. |
