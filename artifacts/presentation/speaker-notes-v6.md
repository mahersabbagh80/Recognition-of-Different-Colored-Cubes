...
Computer vision extracts information from pictures. Our task is object detection: identify each cube's color class and draw a box around it. The example shows human-approved labels, which define the expected answers; it is not a model result. A box gives a position in the image, not a physical distance. Depth measurements are needed for distance and later 3D localization.
